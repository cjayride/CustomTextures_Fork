...
[ThunderStore](https://thunderstore.io/c/valheim/p/cjayride/CustomTextures) | [GitHub](https://github.com/cjayride/CustomTextures_Fork)

# Replace game textures with custom png files.
...
It also now supports layers, so you can apply multiple textures to the same vanilla texture.  

# EXAMPLE

- https://imgur.com/a/Jsgxrms

- Take the image above for a Karve sail, save it as "renderer_sail_full_MainTex.png"

- Put it in the folder cjayride-CustomTextures/CustomTextures

The next time you run the game, it will replace the Karve sail with this image.
  


# Basic Usage

To extract the vanilla textures for reference, I use [Asset Studio](https://ci.appveyor.com/project/Perfare/assetstudio/branch/master/artifacts). The textures are in the **valheim_Data/sharedassets** files.  
...
[Here's some kind person's video on how to use this mod with premade texture packs.](https://youtu.be/gOKhSMAglms)  
...
Property names are usually prefixed by an underscore, so don't forget that too. Also, if you have Windows hiding your file extensions, don't  add **.png** to the end or you'll end up with a file called **\<prefix>_\<identifier>\<propertyName>.png.png** which won't work. Seriously, just have Windows show file extensions, it's dumb that it doesn't. 
...
**_Note:_** _Equipment and world objects are now all considered  **objects**. The mod will still recognize the  **items_**  prefix, just for backwards compatibility.
...
-   **texture**  - used to change all instances of a specific texture of the type  \<suffix>  on all objects and items that use that texture. The  \<identifier>  (see  Identifiers) should be the texture name (not necessarily the exported file name!)
...
-   **armor**  - used to change the chest or legs of armor items - note that the rest of the armor's parts are affected by object  rather than this.
...
[**https://www.reddit.com/r/valheim/comments/lig8ml/spawn_item_command_list/**](https://www.reddit.com/r/valheim/comments/lig8ml/spawn_item_command_list/)  
